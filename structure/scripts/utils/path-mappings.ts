 ...
 */
export function addGenerationHeader(
  content: string,
  sourcePath: string
): string {
 ...
 */
export function validateTransformation(content: string): void {
  // Patterns that should NOT exist after transformation
  const invalidPatterns: { pattern: RegExp; description: string }[] = [
  ...
  for (const { pattern, description } of invalidPatterns) {
    if (pattern.test(content)) {
      throw new Error(
        `Invalid path reference found after transformation: ${description}`
      );
    }
  }
}
 ...
 */
export function getTransformationStats(content: string): Record<string, number> {
